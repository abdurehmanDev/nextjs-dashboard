...
function CompoPercent2(props) {

    const handleClick = (index, active) => {
        const updatedArr = props.percentArr2.map(item => {
          if (item.id === index) {
            if (active) {
        ...
        props.setPercentArr2(updatedArr);
      };
    
    
      return (
        <>
          { props.percentArr2.map((item, i) => (
